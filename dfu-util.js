...
(function ()
{
  ...
  document.addEventListener('DOMContentLoaded', event =>
    {
	  ...
	  downloadButton.addEventListener('click', async function (event)
      {
        ...
        fetch('https://raw.githubusercontent.com/tuftsceeo/SPIKE-html/main/Gecko_combi.bin')
          .then(response =>
          {
            ...
            return response.arrayBuffer(); // Convert response to text
          })
          .then(data =>
          {
            //firmwareFile2 = data
            //console.log("Immediate log:", data);
            //console.log("##########: ", data.length);
            firmwareFile1 = data;//stringToArrayBuffer(data)
      ...
      downloadButton2.addEventListener('click', async function (event)
      {
        ...
        const myTimeout = setTimeout(dfu_upload_2, 3000, device, manifestationTolerant, transferSize, downloadLog, dfu);
  ...
  function stringToArrayBuffer(str) {
    const buf = new ArrayBuffer(str.length); // Create a buffer with the same length as the string
    const bufView = new Uint8Array(buf); // Create a view for the buffer
    for (let i = 0; i < str.length; i++) {
      bufView[i] = str.charCodeAt(i); // Assign each character's char code to the buffer
    }
    return buf;
  }


  async function dfu_upload_1(device, manifestationTolerant, transferSize, downloadLog, dfu)
  {

    console.log("Immediate log:", firmwareFile1);
    console.log("##########_dfu_upload_##########");

    if (device && firmwareFile1 != null)
    {
      ...
      await device.do_download(transferSize, firmwareFile1, manifestationTolerant).then(
        () =>
        {
          logInfo("Done!");
          setLogContext(null);
  ...
  async function dfu_upload_2(device, manifestationTolerant, transferSize, downloadLog, dfu)
  {
